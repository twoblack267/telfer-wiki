...
`.trim();
  ...
  try {
    ...
    execFileSync('python3', [`${process.env.HOME}/.hermes/scripts/check-duplicate-card.py`,
      '--title', `${cardId} — ${title}`, '--id', cardId, '--file', taskFile], { stdio: 'pipe' });
  } catch (e) {
  ...
  try {
    execSync(`python3 "${process.env.HOME}/.hermes/scripts/sync-task-cards-to-board.py" --id ${cardId}`, { cwd: REPO, stdio: 'pipe' });
  } catch (e) {
